...
当前原生若依 Vue 3 主线稳定版本为 **v1.1.4**，包含统一后台 RBAC、未实现接口的明确 501 响应、单机平滑更新流程，以及 Linux x86_64 后端和前端运行包。

| 来源 | 实际含义 |
| --- | --- |
| `v1.1.4` | 当前原生若依主线：在 v1.1.3 基础上加入单机平滑更新及 Linux 运行包 |
| `main` | v1.1.4 发布后继续开发的分支；不会自动改变已发布附件 |
| `v1.1.3` | 历史原生主线：统一后台、RBAC、能力状态修正及 Linux 运行包，不含平滑更新工具 |
| `v1.1.2` | 历史原生基线；源码 ZIP，不含后续 RBAC 和能力状态修正 |
| `v1.2.0` / `v1.3.0` / `v1.4.0` | Whiteyun Vue 历史支线；功能、目录和升级脚本不代表当前 main |

## 获取当前功能

从 [v1.1.4 Release](https://github.com/touensan/ruoyi-go-by/releases/tag/v1.1.4) 下载 Linux 运行包和 `SHA256SUMS`，校验后按 [README](../README.md) 配置。已有部署升级前先备份配置、数据库和上传文件。
...
旧标签和附件保持原样；其内部文档属于历史快照。v1.1.4 包内 `BUILD_INFO.json` 记录源码提交、Go 工具链和前端产物摘要。

## 已核对的标签与附件

| 标签 | 对应源码提交 | 附件 |
| --- | --- | --- |
| [v1.1.4](https://github.com/touensan/ruoyi-go-by/releases/tag/v1.1.4) | 见包内 `BUILD_INFO.json` | ruoyi-go-by-v1.1.4-linux-x86_64.tar.gz, SHA256SUMS |
