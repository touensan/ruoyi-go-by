...
export function parseTime(time: any, pattern?: string): string | null {
  ...
  if (typeof time === 'object') {
    ...
    date = new Date(time)
    // RFC 3339/ISO 8601 values returned by the API must retain their timezone
    // suffix. Only fall back to the legacy slash-normalized form for older
    // browser date strings that are otherwise unparseable.
    if (Number.isNaN(date.getTime()) && typeof time === 'string') {
      const legacyTime = time
        .replace(new RegExp(/-/gm), '/')
        .replace('T', ' ')
        .replace(new RegExp(/\.[\d]{3}/gm), '')
      date = new Date(legacyTime)
    }
  }
