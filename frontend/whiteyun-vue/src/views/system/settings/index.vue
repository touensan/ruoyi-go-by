...
function runMailTest() {
  ...
  testMailSetting({
    ...
    subject: 'APIAuth 邮箱测试',
    body: '这是一封来自 APIAuth 系统配置的 SMTP 测试邮件。'
  }).then(response => {
...
.system-settings {
  ...
  .favicon-upload-card {
    ...
    &.is-warning {
      ...
      background: color-mix(in srgb, var(--el-color-warning) 10%, var(--admin-surface));
    }
  }
  ...
  .favicon-preview {
    ...
    border-radius: 4px;
    background:
      linear-gradient(45deg, var(--admin-surface-soft) 25%, transparent 25%),
      linear-gradient(-45deg, var(--admin-surface-soft) 25%, transparent 25%),
      linear-gradient(45deg, transparent 75%, var(--admin-surface-soft) 75%),
      linear-gradient(-45deg, transparent 75%, var(--admin-surface-soft) 75%);
